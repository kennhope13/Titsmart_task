...
const oldLightboxRegex = /const Lightbox: React\.FC<\{ images: string\[\]; index: number; onClose: \(\) => void; onPrev: \(\) => void; onNext: \(\) => void \}> = \(\{[\s\S]*?\}\) => \([\s\S]*?<span className="absolute bottom-4 left-1\/2 -translate-x-1\/2 rounded-full bg-white\/10 px-3 py-1 text-xs font-bold text-white">[\s\S]*?\{index \+ 1\} \/ \{images\.length\}[\s\S]*?<\/span>[\s\S]*?<\/div>\s*\);/m;
