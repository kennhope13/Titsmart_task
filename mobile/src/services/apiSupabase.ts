...
// Ãnh xáº¡ CamelCase (cá»§a UI) sang snake_case (cá»§a Database)
...
// Ãnh xáº¡ snake_case (cá»§a Database) sang camelCase (cá»§a UI)
...
export const api = {
  ...
  tasks: {
    ...
    update: async (id: string, data: any) => {
      ...
      if (error) {
        if (error.code === 'PGRST116') throw new Error('Dá»¯ liá»‡u khÃ´ng tá»“n táº¡i trÃªn mÃ¡y chá»§ (cÃ³ thá»ƒ Ä‘Ã£ bá»‹ xÃ³a bá»Ÿi ngÆ°á»i khÃ¡c). Vui lÃ²ng F5 táº£i láº¡i trang.');
        if (error.code === 'PGRST204' || String(error.code).includes('400') || String(error.message).includes('column')) {
          delete payload.is_section_header;
          delete payload.section_name;
          delete payload.project_name;
          ...
          if (retryError) {
             if (retryError.code === 'PGRST116') throw new Error('Dá»¯ liá»‡u khÃ´ng tá»“n táº¡i trÃªn mÃ¡y chá»§ (cÃ³ thá»ƒ Ä‘Ã£ bá»‹ xÃ³a bá»Ÿi ngÆ°á»i khÃ¡c). Vui lÃ²ng F5 táº£i láº¡i trang.');
             throw retryError;
          }
          return toCamelCase({ ...data, ...retryResult });
        }
  ...
  materials: {
    ...
    createTransaction: async (data: any) => {
      ...
      // Loáº¡i bá» cÃ¡c trÆ°á»ng khÃ´ng thuá»™c báº£ng inventory_transactions
