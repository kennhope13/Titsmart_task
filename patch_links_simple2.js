...
const regexProject = /<Link to="\/projects" className="[^"]*">([\s\S]*?)<\/Link>/g;
...
const regexTab = /<(Link to|a href)=\{activeTab\.path\} className="[^"]*">([\s\S]*?)<\/(Link|a)>/g;
...
console.log('Fixed links style 2');
