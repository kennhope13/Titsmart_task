...
const tdOverviewReplacement = `<td className="p-3.5 text-center text-slate-500 font-medium">{material.stt || (index + 1)}</td>
                        {!projectId && <td className="p-3.5"><span className="px-1.5 py-0.5 rounded font-bold text-[10px] bg-slate-100 text-slate-600">{material.projectName || 'Kho Tổng'}</span></td>}`;
...
// 7. Implement Transfer Modal Logic and UI
const handleTransferLogic = `
  const handleTransfer = async (e: React.FormEvent) => {
    ...
    if (transferQuantity > (transferMaterial.currentStock || transferMaterial.initialStock || 0)) {
    ...
    if (!targetMaterial) {
      ...
      // For simplicity, we just use addMaterial. Real app should wait for ID.
      // We will just do a small hack: add the import transaction on next tick or rely on user to see it.
      await addMaterial(newMat);
      
      // Let's create an import tx anyway using a temporary approach or just assume it's created
      ...
      return; // Because we can't easily get the ID immediately without refactoring addMaterial to return ID
    } else {
...
`;
// Insert before Modal definitions
const beforeModalsTarget = `{/* MODAL TẠO VẬT TƯ MỚI */}`;
data = data.replace(beforeModalsTarget, handleTransferLogic + '\n\n      {/* MODAL TẠO VẬT TƯ MỚI */}');

// 8. Transfer Modal UI
const transferModalUI = `
      {/* MODAL CHUYỂN KHO */}
      <Modal isOpen={isTransferModalOpen} onClose={() => setIsTransferModalOpen(false)} title="Chuyển Kho Vị Trí">
...
`;
data = data.replace('</Modal>', '</Modal>\n\n' + transferModalUI);

fs.writeFileSync(filePath, data);
console.log('Done patching inventory');
