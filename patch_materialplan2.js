...
// 2. Add Toolbar UI
const toolbarUI = `      <div className="flex border-b border-slate-200 px-4 bg-slate-50 gap-4 sticky top-0 z-10">
        <button
...
`;
const oldTabsStr = \`<div className="flex border-b border-slate-200 px-4 bg-slate-50 gap-4 sticky top-0 z-10">
        <button
          onClick={() => setSubTab('TECH')}
          className={\\\`app-tab-button flex items-center gap-1.5 px-3 py-3 border-b-2 transition-all whitespace-nowrap \\\${subTab === 'TECH' ? 'border-primary text-primary' : 'border-transparent text-slate-500 hover:text-slate-800 hover:border-slate-300'}\\\`}
        >
...
content = content.replace(oldTabsStr, toolbarUI);
...
`;
const oldResolveParentStr = \`    const resolveParentId = (plan: ProjectMaterialPlan): string | undefined => {
      if (plan.stt && plan.stt.includes('.')) {
        const parts = plan.stt.split('.');
        parts.pop();
        const parentStt = parts.join('.');
...
content = content.replace(oldResolveParentStr, filterExecution);
