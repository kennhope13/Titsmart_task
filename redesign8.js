...
const replacementContent = `{selectedProject ? (
...
const startIdx = f.indexOf('<div className="grid grid-cols-1 gap-6 xl:grid-cols-2">');
const endIdx = f.indexOf('</div>', f.indexOf('))}', startIdx)) + 6;

f = f.substring(0, startIdx) + replacementContent + f.substring(endIdx);
...
// Also fix the root div so the page can scroll
f = f.replace('<div className="flex min-h-full flex-1 flex-col bg-slate-100">', '<div className="flex min-h-screen flex-1 flex-col bg-slate-100">');

fs.writeFileSync('web-admin/src/pages/FieldLogsPage.tsx', f, 'utf8');
console.log("Successfully rebuilt FieldLogsPage.tsx correctly.");
