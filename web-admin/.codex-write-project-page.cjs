const fs = require('fs');
const content = String.raw`import React, { useMemo, useState } from 'react';
...
export const ProjectManagementPage: React.FC = () => {
  ...
  const handleCreateProject = (event: React.FormEvent) => {
    ...
    if (pendingProjectTasks.length > 0) {
      ...
      const importedTasks: Omit<Task, 'id'>[] = pendingProjectTasks
        ...
        .map((item, index) => ({
          stt: item.isSectionHeader ? '' : (item.stt || String(index + 1)),
    ...
    triggerToast(` + '`Đã tạo dự án ${newProject.name}`' + `, 'success');
  ...
  return (
    <div className="p-6 space-y-6">
      {toastState.show && <Toast message={toastState.message} type={toastState.type} onClose={() => setToastState((state) => ({ ...state, show: false }))} />}

      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Quản lý Dự án</h1>
          <p className="text-sm text-slate-500 mt-1">Tổng quan thông tin và tiến độ của tất cả các dự án</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <div className="relative">
            <span className="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 text-[18px]">search</span>
            <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Tìm kiếm dự án..." className="pl-9 pr-3 py-2 border border-slate-200 rounded-lg text-sm focus:ring-2 focus:ring-primary focus:outline-none" />
          </div>
          <button type="button" onClick={() => setIsImportProjectModalOpen(true)} className="px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-bold hover:bg-emerald-700 flex items-center gap-2">
            <span className="material-symbols-outlined text-[18px]">upload_file</span> Nhập file
          </button>
          <button type="button" onClick={() => setIsNewProjectModalOpen(true)} className="px-4 py-2 rounded-lg bg-primary text-white text-sm font-bold hover:opacity-90 flex items-center gap-2">
            <span className="material-symbols-outlined text-[18px]">add</span> Dự án mới
          </button>
        </div>
      </div>
            ...
            <article key={project.id} onClick={() => openProjectTasks(project)} role="button" tabIndex={0} onKeyDown={(event) => { if (event.key === 'Enter') openProjectTasks(project); }} className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-slate-500 mt-1">Mã: {project.code}</p>
                </div>
                <button type="button" onClick={(event) => { event.stopPropagation(); setProjectToDelete(project); }} className="w-8 h-8 rounded-lg text-red-500 hover:bg-red-50 flex items-center justify-center" title="Xóa dự án" aria-label={` + '`Xóa dự án ${project.name}`' + `}>
                  <span className="material-symbols-outlined text-[18px]">delete</span>
                </button>
              </div>
              <div className="mt-4 space-y-2 text-sm">
                <div className="flex justify-between gap-3"><span className="text-slate-500">Chủ đầu tư</span><span className="font-semibold text-slate-700 text-right">{project.client || 'Chưa cập nhật'}</span></div>
                <div className="flex justify-between gap-3"><span className="text-slate-500">Địa điểm</span><span className="font-semibold text-slate-700 text-right">{project.location || 'Chưa cập nhật'}</span></div>
                <div className="flex justify-between gap-3"><span className="text-slate-500">PM phụ trách</span><span className="font-semibold text-slate-700 text-right">{project.managerName || 'Chưa phân công'}</span></div>
                <div className="flex justify-between gap-3"><span className="text-slate-500">Trạng thái</span><span className="font-semibold text-slate-700 text-right">{statusLabel[project.status]}</span></div>
              </div>
              <div className="mt-4">
                <div className="flex items-center justify-between text-xs font-bold text-slate-600"><span>Tiến độ thi công</span><span>{progress}%</span></div>
                <div className="h-2 rounded-full bg-slate-100 overflow-hidden mt-2"><div className="h-full bg-primary" style={{ width: `${progress}%` }} /></div>
                <div className="flex justify-between text-xs text-slate-500 mt-2"><span>Đã xong: {completedTasks}</span><span>Còn lại: {Math.max(0, totalTasks - completedTasks)} đầu việc</span></div>
              </div>
            </article>
          );
        })}
      ...
      {displayProjects.length === 0 && (
        <div className="text-center py-16 bg-white border border-dashed border-slate-300 rounded-xl">
          ...
          <h3 className="mt-3 font-bold text-slate-700">{searchQuery ? 'Không có dự án phù hợp' : 'Chưa có dự án nào'}</h3>
          <p className="text-sm text-slate-500 mt-1">{searchQuery ? 'Xóa từ khóa tìm kiếm để xem lại toàn bộ dự án.' : 'Tạo dự án mới hoặc nhập file công trình để hiển thị tại đây.'}</p>
        </div>
      )}

      <Modal isOpen={isImportProjectModalOpen} onClose={() => setIsImportProjectModalOpen(false)} title="Nhập dự án từ file" size="xl">
        <div className="space-y-4">
          <OcrUploadPanel onExtracted={applyImportedProject} />
          <p className="text-xs text-slate-500">File sẽ được đọc để lấy Công trình, Địa điểm công trình và các đầu mục trong bảng. Ảnh và PDF scan mới dùng OCR; Excel, CSV, DOCX và PDF có text sẽ đọc trực tiếp.</p>
        </div>
      </Modal>

      <Modal isOpen={Boolean(projectToDelete)} onClose={() => setProjectToDelete(null)} title="Xóa dự án">
        <div className="space-y-4">
          <p>Bạn chắc chắn muốn xóa dự án <strong>{projectToDelete?.name}</strong>?</p>
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setProjectToDelete(null)} className="px-4 py-2 border border-slate-200 rounded-lg font-semibold text-slate-600 hover:bg-slate-100">Hủy</button>
            <button type="button" onClick={handleDeleteProject} className="px-5 py-2 bg-red-600 text-white rounded-lg font-bold hover:bg-red-700">Xóa dự án</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={isNewProjectModalOpen} onClose={() => setIsNewProjectModalOpen(false)} title="Khởi tạo Dự án / Công trình mới">
        <form onSubmit={handleCreateProject} className="space-y-4">
          <div>
            ...
            <input required value={newProjName} onChange={(e) => setNewProjName(e.target.value)} placeholder="VD: Trạm biến áp 110kV Phước Lý" className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary focus:outline-none" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block font-bold text-slate-700 mb-1">Mã Dự án</label>
              <input value={newProjCode} onChange={(e) => setNewProjCode(e.target.value)} placeholder="Tự sinh nếu bỏ trống" className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary focus:outline-none" />
            </div>
            <div>
              <label className="block font-bold text-slate-700 mb-1">Địa điểm công trình</label>
              <input value={newProjLocation} onChange={(e) => setNewProjLocation(e.target.value)} placeholder="VD: Cần Giuộc, Long An" className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary focus:outline-none" />
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block font-bold text-slate-700 mb-1">Chủ đầu tư</label>
              <input value={newProjClient} onChange={(e) => setNewProjClient(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary focus:outline-none" />
            </div>
            <div>
              <label className="block font-bold text-slate-700 mb-1">Giá trị hợp đồng</label>
              <input value={newProjContractValue} onChange={(e) => setNewProjContractValue(e.target.value)} inputMode="numeric" className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary focus:outline-none" />
            </div>
          </div>
          <div>
            <label className="block font-bold text-slate-700 mb-1">Chỉ huy trưởng</label>
            <select value={newProjManagerId} onChange={(e) => setNewProjManagerId(e.target.value)} className="w-full px-3 py-2 border border-slate-200 rounded-lg bg-white focus:ring-2 focus:ring-primary focus:outline-none">
          ...
          {newProjManagerId === '__NEW__' && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 rounded-lg border border-blue-100 bg-blue-50 p-3">
              <div>
                <label className="block font-bold text-slate-700 mb-1">Tên người mới *</label>
                <input required value={newManagerName} onChange={(e) => setNewManagerName(e.target.value)} placeholder="VD: Kỹ sư Minh" className="w-full px-3 py-2 border border-blue-200 rounded-lg focus:ring-2 focus:ring-primary focus:outline-none" />
              </div>
              <div>
                <label className="block font-bold text-slate-700 mb-1">Chức danh</label>
                <input value={newManagerTitle} onChange={(e) => setNewManagerTitle(e.target.value)} placeholder="VD: Chỉ huy trưởng công trình" className="w-full px-3 py-2 border border-blue-200 rounded-lg focus:ring-2 focus:ring-primary focus:outline-none" />
              </div>
            </div>
          )}
          {pendingProjectTasks.length > 0 && <p className="text-sm text-emerald-700 font-semibold">Khi lưu dự án, hệ thống sẽ đưa {pendingProjectTasks.length} dòng trong bảng vào tab Công việc.</p>}
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" onClick={() => setIsNewProjectModalOpen(false)} className="px-4 py-2 border border-slate-200 rounded-lg font-semibold text-slate-600 hover:bg-slate-100">Hủy</button>
            <button type="submit" className="px-5 py-2 bg-primary text-white rounded-lg font-bold hover:opacity-90">Tạo Dự án</button>
          </div>
        </form>
      </Modal>
    </div>
...
`;
fs.writeFileSync('src/pages/ProjectManagementPage.tsx', content, 'utf8');
