...
// Insert into Cập nhật Phiếu Chi modal
// Looking for the block where the grid of 3 items (ĐVT, Số lượng, Đơn giá) ends
const targetBlock = `<div className="grid grid-cols-3 gap-3">
...
if (code.includes(targetBlock)) {
  code = code.replace(targetBlock, renderAdditionalItemsUI + '\n            ' + targetBlock);
  fs.writeFileSync(file, code, 'utf8');
  console.log("Injected into Cập nhật modal successfully");
} else {
  console.log("Could not find target block for Cập nhật modal");
}
