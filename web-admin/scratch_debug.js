...
async function runImport() {
  ...
  await supabase.from('tasks').delete().eq('project_code', selectedProject);
  await supabase.from('material_plans').delete().eq('project_code', selectedProject);
  await supabase.from('purchasing_plans').delete().eq('project_code', selectedProject);

  const fileName = '2.Phụ lục 01 Bảng giá HĐ TSM_XLĐTĐ - TBA 110kV Trà Ôn.xlsx';
  const data = fs.readFileSync('../documents/' + fileName);
  ...
  console.log(pendingTasks.map(t => t.stt).filter(s => String(s).startsWith('26'))); const { error: e3 } = await supabase.from('tasks').insert(pendingTasks);
