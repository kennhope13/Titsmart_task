...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  return (
    <div className="h-screen bg-slate-50 text-slate-800 flex flex-col font-sans overflow-hidden relative">
      ...
      <div className={`layout-content-wrapper ml-0 ${isSidebarExpanded ? 'md:ml-[170px]' : 'md:ml-[56px]'} flex flex-col h-screen flex-1 overflow-hidden transition-all duration-300 ease-in-out relative pb-[calc(env(safe-area-inset-bottom,0px)+58px)] md:pb-0 pt-[env(safe-area-inset-top,0px)]`}>
        <RealtimeClock />
