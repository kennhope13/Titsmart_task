  ...
  return (
    <div className="flex flex-col flex-1 min-h-full bg-slate-50 relative overflow-y-auto">
      <section className="border-b border-slate-200 bg-white px-6 py-4 flex items-center gap-4">
        ...
        <div><h2 className="page-title text-2xl font-extrabold text-slate-900 border-l-4 border-primary pl-4 uppercase">TÀI KHOẢN</h2></div>
      </section>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-5 flex flex-col items-center text-center">
          <div className="w-20 h-20 rounded-2xl bg-primary text-white flex items-center justify-center text-2xl font-extrabold">{(user?.name || engineer?.name || 'QL').slice(0, 2).toUpperCase()}</div>
          <h3 className="mt-3 text-lg font-extrabold text-slate-900">{user?.name || engineer?.name || 'Quản lý'}</h3>
          <span className="mt-2 px-3 py-1 rounded-full bg-blue-50 text-primary text-xs font-bold">{user?.title || 'Quản lý'}</span>
        </div>

        <div className="lg:col-span-2 bg-white rounded-xl border border-slate-200 shadow-xs p-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            <Info label="Họ tên" value={user?.name || engineer?.name || 'Quản lý'} icon="person" />
            <Info label="Tên đăng nhập" value={user?.username || 'admin'} icon="badge" />
            <Info label="Vai trò" value={user?.title || 'Quản lý'} icon="verified_user" />
            <Info label="Email" value={user?.email || engineer?.email || 'admin@titsmart.vn'} icon="mail" />
            <Info label="Số điện thoại" value={user?.phone || engineer?.phone || 'Chưa cập nhật'} icon="call" />
          </div>
          ...
          <div className="mt-5 pt-4 border-t border-slate-100 flex flex-wrap gap-2 justify-end">
            ...
            <button onClick={handleLogout} className="px-4 py-2 rounded-lg bg-red-600 text-white text-xs font-bold hover:opacity-90"><span className="material-symbols-outlined text-sm align-[-2px] mr-1">logout</span>Đăng xuất</button>
          </div>
        </div>
      </section>
      </div>
