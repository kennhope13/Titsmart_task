              ...
              <div className="space-y-4">
                {logs.map(log => (
                  <div key={log.id} className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                    <div className="flex items-start justify-between gap-3 border-b border-slate-100 bg-slate-50/60 px-4 py-2.5">
                      <div className="min-w-0">
                        <p className="truncate text-xs font-extrabold text-slate-700">
                          {projectName(log.projectCode)}
                        </p>
                        <p className="text-[11px] font-semibold text-slate-400">
                          {formatTimeOnly(log.timestamp)} · {log.images.length} ảnh
                        </p>
                      </div>
                    ...
                    {log.note && (
                      <p className="whitespace-pre-wrap border-b border-slate-100 px-4 py-2.5 text-xs text-slate-600">
                        {log.note}
                      </p>
                    )}

                    <div className="grid grid-cols-2 gap-1 p-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
