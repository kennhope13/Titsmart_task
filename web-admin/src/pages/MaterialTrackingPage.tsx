...
export const MaterialTrackingPage: React.FC = () => {
  ...
  return (
    <div className="flex flex-col flex-1 h-full bg-slate-50 relative overflow-hidden">
            {!projectId && (
        <section className={`border-b border-slate-200 bg-white pl-3 py-4 md:py-0 md:h-12 flex flex-col xl:flex-row justify-between xl:items-center gap-3 pr-[4.5rem]`}>
      ...
      <Modal isOpen={isTransactionModalOpen} onClose={() => setIsTransactionModalOpen(false)} title={transactionType === 'IMPORT' ? 'Tạo Phiếu Nhập Kho' : 'Tạo Phiếu Xuất Kho'}>
        <form onSubmit={handleSubmitTransaction} className="space-y-3 text-xs">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div><label className="block font-bold text-slate-700 mb-1">Số lượng *</label><input type="number" step="any" required min="1" value={txQuantity} onChange={(e) => setTxQuantity(Number(e.target.value))} className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary focus:outline-none bg-white" /></div>
          </div>
          <div>
