...
export const MaterialTrackingPage: React.FC = () => {
  ...
  const handleSyncCodeFromImportLogs = async () => {
    ...
    try {
      ...
      // 1.5. Clean duplicate materials within the SAME project/warehouse
      const projectMaterialMap = new Map<string, Material[]>();
      for (const m of materials) {
        const pKey = `${m.projectCode || 'COMPANY'}:::${cleanCodeString(m.code).toLowerCase()}`;
        if (!projectMaterialMap.has(pKey)) {
          projectMaterialMap.set(pKey, []);
        }
        projectMaterialMap.get(pKey)!.push(m);
      }
  ...
  const handleAddMaterial = async (e: React.FormEvent) => {
    ...
    const newMat = {
      ...
      code: finalCode,
      name: matName.trim(),
      category: addCategory.trim() || 'Vật tư chung',
    ...
    <div className="flex flex-col flex-1 h-full bg-slate-50 relative overflow-hidden">
      {historyMaterial && (
        ...
        />
            ...
            {!projectId && (
        <section className="sticky top-0 z-30 border-b border-slate-200 bg-white pl-3 pr-16 md:pr-20 py-2.5 md:py-0 md:h-12 shadow-sm">
      ...
      <div className="flex-1 flex flex-col min-w-0 min-h-0 overflow-hidden pb-[60px] md:pb-0">
