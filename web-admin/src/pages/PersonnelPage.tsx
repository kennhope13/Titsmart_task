...
<div className="border-t border-slate-100 my-2"></div>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <label className="flex items-center gap-2 text-[13px] text-slate-700 font-medium"><input type="checkbox" checked={permissions.includes('VIEW_ISSUES')} onChange={(e) => e.target.checked ? setPermissions(p => [...p, 'VIEW_ISSUES']) : setPermissions(p => p.filter(x => x !== 'VIEW_ISSUES'))} className="accent-primary w-3.5 h-3.5"/>Xem danh sách sự cố</label>
                  <label className="flex items-center gap-2 text-[13px] text-slate-700 font-medium"><input type="checkbox" checked={permissions.includes('MANAGE_ISSUES')} onChange={(e) => e.target.checked ? setPermissions(p => [...p, 'MANAGE_ISSUES']) : setPermissions(p => p.filter(x => x !== 'MANAGE_ISSUES'))} className="accent-primary w-3.5 h-3.5"/>Báo cáo & xử lý sự cố</label>
                </div>
                {/* VẬT TƯ */}
                <div className="space-y-1">
