...
const hasSyncedRef = useRef(false);
  useEffect(() => {
    ...
    tasks.forEach(task => {
      ...
      if (matchingMaterial) {
        const matUpdates: Record<string, any> = {};
        if (hasPurchase && norm(matchingMaterial.orderedStatus) !== norm(task.purchaseStatus)) {
          matUpdates.orderedStatus = task.purchaseStatus;
        }
      ...
      if (hasPurchase) {
        ...
        if (matchingPurchasing && norm(matchingPurchasing.orderStatus) !== norm(task.purchaseStatus)) {
          updatePurchasingPlan(matchingPurchasing.id, { orderStatus: task.purchaseStatus });
          syncCount++;
        }
      }
    });
    ...
    if (syncCount > 0) {
      console.log(`[TaskSync] Auto-synced ${syncCount} status(es) from Tasks to MaterialPlan/Purchasing`);
    }
  }, [tasks, materialPlans, purchasingPlans]);
      ...
      <div className="relative flex-shrink-0">
        ...
        {showNotifPopover && (
          <div className="absolute top-full right-0 mt-2 w-72 bg-white rounded-xl shadow-xl border border-slate-200 overflow-hidden z-50">
            ...
            <div className="max-h-80 overflow-y-auto custom-scrollbar divide-y divide-slate-200">
                ...
                groupedTasks.filter((t) => {
                  ...
                  return (
                    <tr key={t.id} className={rowClass} onDoubleClick={() => handleOpenEditModal(t)}>
                      ...
                      <td className="py-1.5 px-1 text-center font-mono font-semibold text-slate-900 whitespace-nowrap border-r border-slate-200">
                        {editingCell?.id === t.id && editingCell?.field === 'volume' ? (
                          <input type="number" step="any" value={tempValue} onChange={(e) => setTempValue(e.target.value)} onBlur={() => saveEditing(t)} onKeyDown={(e) => { if (e.key === 'Enter') saveEditing(t); if (e.key === 'Escape') setEditingCell(null); }} autoFocus className="w-full text-center border rounded px-0.5 py-0.5 bg-white text-slate-900 font-bold focus:outline-primary text-[10px]" />
                        ) : (
            ...
            <>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input type="number" step="any" value={volume} onChange={(e) => setVolume(Number(e.target.value))} className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary focus:outline-none bg-white font-mono" />
                </div>
                <div>
