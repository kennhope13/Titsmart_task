...
export const TransactionModal: React.FC<TransactionModalProps> = ({ isOpen, type, materials, defaultMaterialId, onClose, onSave }) => {
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    if (type === 'EXPORT' && quantity > materialCurrentStock(selectedMaterial)) {
      const confirmed = window.confirm(`S? lu?ng xu?t (${quantity}) l?n hon t?n kho hi?n t?i (${materialCurrentStock(selectedMaterial)}). B?n c� mu?n xu?t kho �m kh�ng?`);
      if (!confirmed) return;
    }
      ...
      <form onSubmit={handleSubmit} className="space-y-3 text-xs">
        ...
        {selectedMaterial && (
          <div className="grid grid-cols-3 gap-2 bg-slate-50 border border-slate-200 rounded-lg p-3 text-[11px] text-slate-600">
            ...
            <div><b>Quy c�ch:</b> {selectedMaterial.specs || selectedMaterial.englishName || '-'}</div>
            <div><b>�VT:</b> {selectedMaterial.unit || '-'}</div>
          </div>
        )}
        <div className="grid grid-cols-2 gap-3">
          <Field label={`Ng�y ${type === 'IMPORT' ? 'nh?p' : 'xu?t'} *`}><input type="date" required value={date} onChange={(event) => setDate(event.target.value)} className={inputClass} /></Field>
        ...
        <Field label={type === 'IMPORT' ? 'Ngu?n Nh?p / D? �n Du *' : 'M� D? �n / T�n C�ng Tr�nh *'}><input required value={sourceOrProject} onChange={(event) => setSourceOrProject(event.target.value)} className={inputClass} /></Field>
        ...
        <Field label="Ghi Ch�"><input value={notes} onChange={(event) => setNotes(event.target.value)} className={inputClass} /></Field>
