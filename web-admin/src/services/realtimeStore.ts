...
const normalizeMaterialPlan = (plan: any): ProjectMaterialPlan => ({
  ...
  techSpecStatus: plan.techSpecStatus ?? plan.tech_spec_status ?? (String(plan.notes || '').match(/\[tech-status:([^\]]+)\]/i)?.[1] || ''),
...
const normalizePurchasingPlan = (plan: any): ProjectPurchasing => ({
  ...
  costPaidAmount: Number(plan.costPaidAmount ?? plan.cost_paid_amount ?? 0),
  costPaymentDate: plan.costPaymentDate ?? plan.cost_payment_date ?? '',
  costPaymentMethod: plan.costPaymentMethod ?? plan.cost_payment_method ?? '',
  costPayerName: plan.costPayerName ?? plan.cost_payer_name ?? '',
