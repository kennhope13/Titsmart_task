...
const data = [
  ...
  { id: '4', stt: '4.19.1', notes: '[contractor] | [order:00153]', parentId: '2' },
  { id: '5', stt: 'II.1', notes: '[contractor]', parentId: '2' }
];
...
const numericSttParts = (stt) => String(stt || '').split('.').map(s => { const num = parseInt(s, 10); return isNaN(num) ? s : num; });
const romanToInt = (s) => {
  const map = { I: 1, V: 5, X: 10, L: 50, C: 100, D: 500, M: 1000 };
  let total = 0, upper = s.toUpperCase();
  for (let i = 0; i < upper.length; i++) {
    const cur = map[upper[i]] ?? 0, nxt = map[upper[i + 1]] ?? 0;
...
[...data].filter(r => isSectionRow(r)).sort((a, b) => {
...
console.log('flattened:', flattened.map(f => f.stt + ' (secKey: ' + f._sectionKey + ', depth: ' + f.depth + ')'));
